            ...
            if (message.content.startsWith(this.client.config.PREFIX)) {
                ...
                if (command) {
                    ...
                    if (messageArgs.length > 0) {
                        if (!command.args.some(arg => messageArgs.includes(arg))) return message.channel.send(`Argument ${messageArgs} is not allowed.`);
                    }
                    
                    if (command.numberOfArgs > 0 && command.numberOfArgs > messageArgs.length) return message.channel.send(`Wrong number or arguments. Use minimum ${command.numberOfArgs}.`);
